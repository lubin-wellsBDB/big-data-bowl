...
momentum_csv <- read.csv("momentum.csv")
...
allowed_yards_after_tackle_csv <- read.csv("yata.csv")
allowed_yards_after_tackle_csv_selected <- allowed_yards_after_tackle_csv %>%
  select(tacklerId, avgYata) %>%
  rename("nflId" = "tacklerId",
         "avgAyat" = "avgYata")
...
merged_data <- merge(merge(merge(merge(merge(merge(merge(momentum_csv_selected, tackle_over_expectation_csv_selected, by = "nflId")
                                 ...
                                 allowed_yards_after_tackle_csv_selected, by = "nflId"), player_load_fatigue_csv_selected, by = "nflId"), tackle_range_csv_selected, by = "nflId")
...
ayat <- allowed_yards_after_tackle_csv_selected %>%
  filter(nflId %in% players_ids) %>%
  filter(nflId %in% players_ids2)
...
ayat_normalized <- ayat
ayat_normalized$avgAyat <- percentile_normalize(ayat$avgAyat) - .5
...
all_normalized_data <- merge(merge(merge(merge(merge(merge(merge(momentum_normalized, tackle_oe_normalized, by = "nflId")
                                         ...
                                         ayat_normalized, by = "nflId"), fatigue_normalized, by = "nflId"), range_normalized, by = "nflId")

#Metric testing to see how it looked if we weighed all attributes evenly
all_normalized_data_metric <- all_normalized_data %>%
  mutate(final_metric = tackle_range_metric*(1/8)+avgImpulse*(1/8) + avg_tackle_oe*(1/8) + avgSTRAIN*(1/8) + mean_tackle_value*(1/8) + athleticism_score*(1/8) + avgAyat*(1/8) + fatigue*(1/8))
...
ayat_normalized2 <- ayat
ayat_normalized2$avgAyat <- percentile_normalize(ayat$avgAyat) * 100
...
all_normalized_data_perc <- merge(merge(merge(merge(merge(merge(merge(momentum_normalized2, tackle_oe_normalized2, by = "nflId")
                                              ...
                                              ayat_normalized2, by = "nflId"), fatigue_normalized2, by = "nflId"), range_normalized2, by = "nflId")
